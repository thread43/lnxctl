...
func GetClusterroleYaml(response http.ResponseWriter, request *http.Request) {
	var err error
	...
	clusterrole, err = clientset.RbacV1().ClusterRoles().Get(context.Background(), name, meta_v1.GetOptions{})
	util.Raise(err)

	var clusterrole2 []byte
	clusterrole2, err = yaml.Marshal(clusterrole)
	util.Raise(err)

	util.Api(response, 200, string(clusterrole2))
}
