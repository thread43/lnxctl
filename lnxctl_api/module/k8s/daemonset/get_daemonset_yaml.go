...
func GetDaemonsetYaml(response http.ResponseWriter, request *http.Request) {
	var err error
	...
	var daemonset *apps_v1.DaemonSet
	daemonset, err = clientset.AppsV1().DaemonSets(namespace).Get(context.Background(), name, meta_v1.GetOptions{})
	util.Raise(err)

	var daemonset2 []byte
	daemonset2, err = yaml.Marshal(daemonset)
	util.Raise(err)

	util.Api(response, 200, string(daemonset2))
}
