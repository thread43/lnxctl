...
func GetEndpointYaml(response http.ResponseWriter, request *http.Request) {
	var err error
	...
	var endpoint *core_v1.Endpoints
	endpoint, err = clientset.CoreV1().Endpoints(namespace).Get(context.Background(), name, meta_v1.GetOptions{})
	util.Raise(err)
