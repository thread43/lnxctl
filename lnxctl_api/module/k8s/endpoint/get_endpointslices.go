...
func GetEndpointslices(response http.ResponseWriter, request *http.Request) {
	var err error
	...
	var endpoint_list *discovery_v1.EndpointSliceList
	...
	for _, item = range endpoint_list.Items {
		...
		// EndpointSlices diffs from Endpoints, the name has a hash suffix
		var name string
		name = item.Name
		...
		var endpoint discovery_v1.Endpoint
		for _, endpoint = range item.Endpoints {
			var address string
			for _, address = range endpoint.Addresses {
				var port discovery_v1.EndpointPort
				for _, port = range item.Ports {
					// log.Printf("%+v\n", *port.Port)
					// log.Printf("%+v\n", *port.Protocol)
					address_ports = append(address_ports, fmt.Sprintf("%s:%d", address, *port.Port))
				}
			}
		}
