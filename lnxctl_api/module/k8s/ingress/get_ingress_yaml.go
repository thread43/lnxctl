...
func GetIngressYaml(response http.ResponseWriter, request *http.Request) {
	var err error
	...
	var ingress *networking_v1.Ingress
	ingress, err = clientset.NetworkingV1().Ingresses(namespace).Get(context.Background(), name, meta_v1.GetOptions{})
	util.Raise(err)

	var ingress2 []byte
	ingress2, err = yaml.Marshal(ingress)
	util.Raise(err)

	util.Api(response, 200, string(ingress2))
}
