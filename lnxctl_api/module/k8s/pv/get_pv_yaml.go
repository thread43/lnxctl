...
func GetPvYaml(response http.ResponseWriter, request *http.Request) {
	var err error
	...
	var pv *core_v1.PersistentVolume
	pv, err = clientset.CoreV1().PersistentVolumes().Get(context.Background(), name, meta_v1.GetOptions{})
	util.Raise(err)

	var secret2 []byte
	secret2, err = yaml.Marshal(pv)
	util.Raise(err)

	util.Api(response, 200, string(secret2))
}
