...
func GetPvcYaml(response http.ResponseWriter, request *http.Request) {
	var err error
	...
	pvc, err = clientset.CoreV1().PersistentVolumeClaims(namespace).Get(context.Background(), name, meta_v1.GetOptions{})
	util.Raise(err)

	var pvc2 []byte
	pvc2, err = yaml.Marshal(pvc)
	util.Raise(err)

	util.Api(response, 200, string(pvc2))
}
