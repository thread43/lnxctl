...
func GetReplicasetYaml(response http.ResponseWriter, request *http.Request) {
	var err error
	...
	replicaset, err = clientset.AppsV1().ReplicaSets(namespace).Get(context.Background(), name, meta_v1.GetOptions{})
	util.Raise(err)

	var replicaset2 []byte
	replicaset2, err = yaml.Marshal(replicaset)
	util.Raise(err)

	util.Api(response, 200, string(replicaset2))
}
