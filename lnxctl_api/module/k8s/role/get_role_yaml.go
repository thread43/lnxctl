...
func GetRoleYaml(response http.ResponseWriter, request *http.Request) {
	var err error
	...
	var role *rbac_v1.Role
	role, err = clientset.RbacV1().Roles(namespace).Get(context.Background(), name, meta_v1.GetOptions{})
	util.Raise(err)

	var role2 []byte
	role2, err = yaml.Marshal(role)
	util.Raise(err)

	util.Api(response, 200, string(role2))
}
