...
func GetRolebindingYaml(response http.ResponseWriter, request *http.Request) {
	var err error
	...
	var rolebinding *rbac_v1.RoleBinding
	rolebinding, err = clientset.RbacV1().RoleBindings(namespace).Get(context.Background(), name, meta_v1.GetOptions{})
	util.Raise(err)

	var rolebinding2 []byte
	rolebinding2, err = yaml.Marshal(rolebinding)
	util.Raise(err)

	util.Api(response, 200, string(rolebinding2))
}
