...
func GetSecretYaml(response http.ResponseWriter, request *http.Request) {
	var err error
	...
	secret, err = clientset.CoreV1().Secrets(namespace).Get(context.Background(), name, meta_v1.GetOptions{})
	util.Raise(err)

	var secret2 []byte
	secret2, err = yaml.Marshal(secret)
	util.Raise(err)
