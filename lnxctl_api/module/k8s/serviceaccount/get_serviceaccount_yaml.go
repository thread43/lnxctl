...
func GetServiceaccountYaml(response http.ResponseWriter, request *http.Request) {
	var err error
	...
	var serviceaccount *core_v1.ServiceAccount
	serviceaccount, err = clientset.CoreV1().ServiceAccounts(namespace).Get(context.Background(), name, meta_v1.GetOptions{})
	util.Raise(err)

	var serviceaccount2 []byte
	serviceaccount2, err = yaml.Marshal(serviceaccount)
	util.Raise(err)

	util.Api(response, 200, string(serviceaccount2))
}
