...
func GetStorageclassYaml(response http.ResponseWriter, request *http.Request) {
	var err error
	...
	var storageclass *storage_v1.StorageClass
	storageclass, err = clientset.StorageV1().StorageClasses().Get(context.Background(), name, meta_v1.GetOptions{})
	util.Raise(err)

	var storageclass2 []byte
	storageclass2, err = yaml.Marshal(storageclass)
	util.Raise(err)

	util.Api(response, 200, string(storageclass2))
}
