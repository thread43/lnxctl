...
function ClusterroleYaml() {
  ...
  async function init() {
    try {
      const clusterrole = storeClusterrole;

      setStateLoading(true);

      const response = await api.get_clusterrole_yaml(clusterrole);
  ...
  return (
    <>
      <Modal
        ...
        width={1000}
        open={storeClusterroleYamlVisible}
        loading={stateLoading}
        onCancel={() => dispatch(store.setClusterroleYamlVisible(false))}
        footer={[
          <Button key="close" onClick={() => dispatch(store.setClusterroleYamlVisible(false))}>Close</Button>,
        ]}
      >
        <div className="MyContentHeader">
          <span className="MyContentHeaderTitle">
            {storeClusterrole.name}
          </span>
          <Space>
...
export default ClusterroleYaml;
