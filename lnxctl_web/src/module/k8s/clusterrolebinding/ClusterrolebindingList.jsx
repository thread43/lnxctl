...
function ClusterrolebindingList() {
  ...
  const storeClusterrolebindings = useSelector(store.getClusterrolebindings);
  const storeClusterrolebindingTableLoading = useSelector(store.getClusterrolebindingTableLoading);
  ...
  async function init(context) {
    ...
    try {
      dispatch(store.setClusterrolebindingTableLoading(true));
      ...
      const response2 = await api.get_clusterrolebindings(cluster_id);
      dispatch(store.setClusterrolebindings(response2.data.data));
    } catch (error) {
      console.error(error);
      message.error(error.message);
    } finally {
      dispatch(store.setClusterrolebindingTableLoading(false));
    }
  }
  ...
  async function changeCluster(value) {
    ...
    dispatch(commonStore.setContext({cluster_id}));
    dispatch(store.setClusterrolebindings([]));

    try {
      dispatch(store.setClusterrolebindingTableLoading(true));
      const response = await api.get_clusterrolebindings(cluster_id);
      dispatch(store.setClusterrolebindings(response.data.data));
    } catch (error) {
      console.error(error);
      message.error(error.message);
    } finally {
  ...
  function getClusterrolebinding(clusterrolebinding) {
    dispatch(store.setClusterrolebinding(clusterrolebinding));
    dispatch(store.setClusterrolebindingDetailVisible(true));
  }

  function getClusterrolebindingYaml(clusterrolebinding) {
    dispatch(store.setClusterrolebinding(clusterrolebinding));
    dispatch(store.setClusterrolebindingYamlVisible(true));
  }
    ...
    <>
      ...
      <div className="MyContentBlock">
        ...
        <Table
          rowKey="name"
          columns={columns}
          dataSource={storeClusterrolebindings}
          loading={storeClusterrolebindingTableLoading}
