...
function ConfigmapList() {
  ...
  const storeConfigmaps = useSelector(store.getConfigmaps);
  const storeConfigmapTableLoading = useSelector(store.getConfigmapTableLoading);
  ...
  async function init(context) {
    ...
    try {
      dispatch(store.setConfigmapTableLoading(true));
      ...
      const response2 = await api.get_configmaps(cluster_id, namespace);
      dispatch(store.setConfigmaps(response2.data.data));
      ...
      dispatch(store.setConfigmapTableLoading(false));
    }
  }
  ...
  async function changeCluster(value) {
    ...
    dispatch(store.setConfigmaps([]));

    try {
      dispatch(store.setConfigmapTableLoading(true));

      const response = await api.get_configmaps(cluster_id, namespace);
      dispatch(store.setConfigmaps(response.data.data));
  ...
  function getConfigmap(configmap) {
    dispatch(store.setConfigmap(configmap));
    dispatch(store.setConfigmapDetailVisible(true));
  }

  function getConfigmapYaml(configmap) {
    dispatch(store.setConfigmap(configmap));
    dispatch(store.setConfigmapYamlVisible(true));
  }
    ...
    <>
      ...
      <div className="MyContentBlock">
        <div className="MyContentHeader">
          <span className="MyContentHeaderTitle">ConfigMap List</span>
          <Space>
        ...
        <Table
          ...
          columns={columns}
          dataSource={storeConfigmaps}
          loading={storeConfigmapTableLoading}
