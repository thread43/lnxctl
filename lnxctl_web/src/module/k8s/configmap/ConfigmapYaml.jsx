...
function ConfigmapYaml() {
  ...
  async function init() {
    try {
      const configmap = storeConfigmap;

      setStateLoading(true);

      const response = await api.get_configmap_yaml(configmap);
  ...
  return (
    <>
      <Modal
        ...
        width={1000}
        open={storeConfigmapYamlVisible}
      ...
      >
        <div className="MyContentHeader">
          <span className="MyContentHeaderTitle">
            {storeConfigmap.namespace} / {storeConfigmap.name}
          </span>
          <Space>
