...
function DaemonsetList() {
  ...
  const storeDaemonsets = useSelector(store.getDaemonsets);
  const storeDaemonsetTableLoading = useSelector(store.getDaemonsetTableLoading);
  ...
  async function init(context) {
    ...
    try {
      dispatch(store.setDaemonsetTableLoading(true));
      ...
      const response2 = await api.get_daemonsets(cluster_id, namespace);
      dispatch(store.setDaemonsets(response2.data.data));
      ...
      dispatch(store.setDaemonsetTableLoading(false));
    }
  }
  ...
  async function changeCluster(value) {
    ...
    dispatch(store.setDaemonsets([]));

    try {
      dispatch(store.setDaemonsetTableLoading(true));

      const response = await api.get_daemonsets(cluster_id, namespace);
      dispatch(store.setDaemonsets(response.data.data));
  ...
  function getDaemonset(daemonset) {
    dispatch(store.setDaemonset(daemonset));
    dispatch(store.setDaemonsetDetailVisible(true));
  }

  function getDaemonsetYaml(daemonset) {
    dispatch(store.setDaemonset(daemonset));
    dispatch(store.setDaemonsetYamlVisible(true));
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      sortDirections: ['ascend', 'descend'],
      render: (text, record) => (
        <Button type="link" className="ButtonLink" onClick={() => getDaemonset(record)}>{text}</Button>
      ),
    },
    {
      key: 'desired_number_scheduled',
      title: 'Desired',
      dataIndex: 'desired_number_scheduled',
    },
    {
      key: 'current_number_scheduled',
      title: 'Current',
      dataIndex: 'current_number_scheduled',
    },
    {
      key: 'number_ready',
      title: 'Ready',
      dataIndex: 'number_ready',
    },
    {
      key: 'updated_number_scheduled',
      title: 'Up To Date',
      dataIndex: 'updated_number_scheduled',
    },
    {
    ...
    },
    {
      ...
      render: (text, record) => (
        <div style={{display: 'flex', alignItems: 'center'}}>
          <Button type="link" className="ButtonLink" onClick={() => getDaemonsetYaml(record)}>YAML</Button>
        </div>
      ),
    },
  ];
    ...
    <>
      ...
      <div className="MyContentBlock">
        ...
        <Table
          rowKey="name"
          columns={columns}
          dataSource={storeDaemonsets}
          loading={storeDaemonsetTableLoading}
