...
function DaemonsetYaml() {
  ...
  async function init() {
    try {
      const daemonset = storeDaemonset;

      setStateLoading(true);

      const response = await api.get_daemonset_yaml(daemonset);
  ...
  return (
    <>
      <Modal
        ...
        width={1000}
        open={storeDaemonsetYamlVisible}
        loading={stateLoading}
        onCancel={() => dispatch(store.setDaemonsetYamlVisible(false))}
        footer={[
          <Button key="close" onClick={() => dispatch(store.setDaemonsetYamlVisible(false))}>Close</Button>,
        ]}
      >
