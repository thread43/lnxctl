...
function EndpointList() {
  ...
  const storeEndpoints = useSelector(store.getEndpoints);
  const storeEndpointTableLoading = useSelector(store.getEndpointTableLoading);
  ...
  async function init(context) {
    ...
    try {
      dispatch(store.setEndpointTableLoading(true));
      ...
      const response2 = await api.get_endpoints(cluster_id, namespace);
      dispatch(store.setEndpoints(response2.data.data));
      ...
      dispatch(store.setEndpointTableLoading(false));
    }
  }
  ...
  async function changeCluster(value) {
    ...
    dispatch(store.setEndpoints([]));

    try {
      dispatch(store.setEndpointTableLoading(true));

      const response = await api.get_endpoints(cluster_id, namespace);
      dispatch(store.setEndpoints(response.data.data));
  ...
  function getEndpoint(endpoint) {
    dispatch(store.setEndpoint(endpoint));
    dispatch(store.setEndpointDetailVisible(true));
  }

  function getEndpointYaml(endpoint) {
    dispatch(store.setEndpoint(endpoint));
    dispatch(store.setEndpointYamlVisible(true));
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      sortDirections: ['ascend', 'descend'],
      render: (text, record) => (
        <Button type="link" className="ButtonLink" onClick={() => getEndpoint(record)}>{text}</Button>
      ),
    },
    {
      key: 'address_ports',
      title: 'Endpoints',
      dataIndex: 'address_ports',
      render: (text) => (
        <>
          {text.map((item, index) => (
            <div key={index}>
    ...
    },
    {
      ...
      render: (text, record) => (
        <div style={{display: 'flex', alignItems: 'center'}}>
          <Button type="link" className="ButtonLink" onClick={() => getEndpointYaml(record)}>YAML</Button>
        </div>
      ),
    },
  ];
    ...
    <>
      ...
      <div className="MyContentBlock">
        <div className="MyContentHeader">
          <span className="MyContentHeaderTitle">Endpoint List</span>
          <Space>
        ...
        <Table
          rowKey="name"
          columns={columns}
          dataSource={storeEndpoints}
          loading={storeEndpointTableLoading}
