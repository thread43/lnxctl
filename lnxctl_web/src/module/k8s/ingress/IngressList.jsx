...
function IngressList() {
  ...
  const storeIngresses = useSelector(store.getIngresses);
  const storeIngressTableLoading = useSelector(store.getIngressTableLoading);
  ...
  async function init(context) {
    ...
    try {
      dispatch(store.setIngressTableLoading(true));
      ...
      const response2 = await api.get_ingresses(cluster_id, namespace);
      dispatch(store.setIngresses(response2.data.data));
      ...
      dispatch(store.setIngressTableLoading(false));
    }
  }
  ...
  async function changeCluster(value) {
    ...
    dispatch(store.setIngresses([]));

    try {
      dispatch(store.setIngressTableLoading(true));

      const response = await api.get_ingresses(cluster_id, namespace);
      dispatch(store.setIngresses(response.data.data));
  ...
  function getIngress(ingress) {
    dispatch(store.setIngress(ingress));
    dispatch(store.setIngressDetailVisible(true));
  }

  function getIngressYaml(ingress) {
    dispatch(store.setIngress(ingress));
    dispatch(store.setIngressYamlVisible(true));
  }
    ...
    },
    {
      ...
      render: (text, record) => (
        <div style={{display: 'flex', alignItems: 'center'}}>
          <Button type="link" className="ButtonLink" onClick={() => getIngressYaml(record)}>YAML</Button>
        </div>
      ),
    },
  ];
    ...
    <>
      ...
      <div className="MyContentBlock">
        <div className="MyContentHeader">
          <span className="MyContentHeaderTitle">Ingress List</span>
          <Space>
        ...
        <Table
          ...
          columns={columns}
          dataSource={storeIngresses}
          loading={storeIngressTableLoading}
