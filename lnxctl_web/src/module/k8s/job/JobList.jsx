...
function JobList() {
  ...
  const storeJobs = useSelector(store.getJobs);
  const storeJobTableLoading = useSelector(store.getJobTableLoading);
  ...
  async function init(context) {
    ...
    try {
      dispatch(store.setJobTableLoading(true));
      ...
      const response2 = await api.get_jobs(cluster_id, namespace);
      dispatch(store.setJobs(response2.data.data));
      ...
      dispatch(store.setJobTableLoading(false));
    }
  }
  ...
  async function changeCluster(value) {
    ...
    dispatch(store.setJobs([]));

    try {
      dispatch(store.setJobTableLoading(true));

      const response = await api.get_jobs(cluster_id, namespace);
      dispatch(store.setJobs(response.data.data));
  ...
  function getJob(job) {
    dispatch(store.setJob(job));
    dispatch(store.setJobDetailVisible(true));
  }

  function getJobYaml(job) {
    dispatch(store.setJob(job));
    dispatch(store.setJobYamlVisible(true));
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      sortDirections: ['ascend', 'descend'],
      render: (text, record) => (
        <Button type="link" className="ButtonLink" onClick={() => getJob(record)}>{text}</Button>
      ),
    },
    {
      key: 'status',
    ...
    },
    {
      ...
      render: (text, record) => (
        <div style={{display: 'flex', alignItems: 'center'}}>
          <Button type="link" className="ButtonLink" onClick={() => getJobYaml(record)}>YAML</Button>
        </div>
      ),
    },
  ];
    ...
    <>
      ...
      <div className="MyContentBlock">
        <div className="MyContentHeader">
          <span className="MyContentHeaderTitle">Job List</span>
          <Space>
        ...
        <Table
          rowKey="name"
          columns={columns}
          dataSource={storeJobs}
          loading={storeJobTableLoading}
