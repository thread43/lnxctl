...
function PvList() {
  ...
  const storePvs = useSelector(store.getPvs);
  const storePvTableLoading = useSelector(store.getPvTableLoading);
  ...
  async function init(context) {
    ...
    try {
      dispatch(store.setPvTableLoading(true));
      ...
      const response2 = await api.get_pvs(cluster_id);
      dispatch(store.setPvs(response2.data.data));
    } catch (error) {
      console.error(error);
      message.error(error.message);
    } finally {
      dispatch(store.setPvTableLoading(false));
    }
  }
  ...
  async function changeCluster(value) {
    ...
    dispatch(commonStore.setContext({cluster_id}));
    dispatch(store.setPvs([]));

    try {
      dispatch(store.setPvTableLoading(true));

      const response = await api.get_pvs(cluster_id);
      dispatch(store.setPvs(response.data.data));
    } catch (error) {
      console.error(error);
      message.error(error.message);
    } finally {
  ...
  function getPv(pv) {
    dispatch(store.setPv(pv));
    dispatch(store.setPvDetailVisible(true));
  }

  function getPvYaml(pv) {
    dispatch(store.setPv(pv));
    dispatch(store.setPvYamlVisible(true));
  }
  ...
  const columns = [
    {
      ...
      render: (text, record) => (
        <Button type="link" className="ButtonLink" onClick={() => getPv(record)}>{text}</Button>
      ),
    },
    {
      key: 'capacity',
    ...
    },
    {
      ...
      render: (text, record) => (
        <div style={{display: 'flex', alignItems: 'center'}}>
          <Button type="link" className="ButtonLink" onClick={() => getPvYaml(record)}>YAML</Button>
        </div>
      ),
    },
  ];
    ...
    <>
      ...
      <div className="MyContentBlock">
        <div className="MyContentHeader">
          <span className="MyContentHeaderTitle">PV List</span>
          <Space>
