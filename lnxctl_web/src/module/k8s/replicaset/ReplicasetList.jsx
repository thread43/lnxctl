...
function ReplicasetList() {
  ...
  const storeReplicasets = useSelector(store.getReplicasets);
  const storeReplicasetTableLoading = useSelector(store.getReplicasetTableLoading);
  ...
  async function init(context) {
    ...
    try {
      dispatch(store.setReplicasetTableLoading(true));
      ...
      const response2 = await api.get_replicasets(cluster_id, namespace);
      dispatch(store.setReplicasets(response2.data.data));
      ...
      dispatch(store.setReplicasetTableLoading(false));
    }
  }
  ...
  async function changeCluster(value) {
    ...
    dispatch(store.setReplicasets([]));

    try {
      dispatch(store.setReplicasetTableLoading(true));

      const response = await api.get_replicasets(cluster_id, namespace);
      dispatch(store.setReplicasets(response.data.data));
  ...
  function getReplicaset(replicaset) {
    dispatch(store.setReplicaset(replicaset));
    dispatch(store.setReplicasetDetailVisible(true));
  }

  function getReplicasetYaml(replicaset) {
    dispatch(store.setReplicaset(replicaset));
    dispatch(store.setReplicasetYamlVisible(true));
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      sortDirections: ['ascend', 'descend'],
      render: (text, record) => (
        <Button type="link" className="ButtonLink" onClick={() => getReplicaset(record)}>{text}</Button>
      ),
    },
    {
      key: 'spec_replicas',
      title: 'Desired',
      dataIndex: 'spec_replicas',
    },
    {
      key: 'status_replicas',
      title: 'Current',
    ...
    },
    {
      ...
      render: (text, record) => (
        <div style={{display: 'flex', alignItems: 'center'}}>
          <Button type="link" className="ButtonLink" onClick={() => getReplicasetYaml(record)}>YAML</Button>
        </div>
      ),
    },
  ];
    ...
    <>
      ...
      <div className="MyContentBlock">
        <div className="MyContentHeader">
          <span className="MyContentHeaderTitle">ReplicaSet List</span>
          <Space>
        ...
        <Table
          rowKey="name"
          columns={columns}
          dataSource={storeReplicasets}
          loading={storeReplicasetTableLoading}
