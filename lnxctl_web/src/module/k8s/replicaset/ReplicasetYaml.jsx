...
function ReplicasetYaml() {
  ...
  async function init() {
    try {
      const replicaset = storeReplicaset;

      setStateLoading(true);

      const response = await api.get_replicaset_yaml(replicaset);
  ...
  return (
    <>
      <Modal
        ...
        width={1000}
        open={storeReplicasetYamlVisible}
      ...
      >
        <div className="MyContentHeader">
          <span className="MyContentHeaderTitle">
            {storeReplicaset.namespace} / {storeReplicaset.name}
          </span>
          <Space>
