...
function RoleList() {
  ...
  async function init(context) {
    ...
    try {
      ...
      const response2 = await api.get_roles(cluster_id, namespace);
      dispatch(store.setRoles(response2.data.data));
  ...
  async function changeCluster(value) {
    ...
    dispatch(store.setRoles([]));

    try {
      dispatch(store.setRoleTableLoading(true));

      const response = await api.get_roles(cluster_id, namespace);
  ...
  function getRole(role) {
    dispatch(store.setRole(role));
    dispatch(store.setRoleDetailVisible(true));
  }

  function getRoleYaml(role) {
    dispatch(store.setRole(role));
    dispatch(store.setRoleYamlVisible(true));
  }
