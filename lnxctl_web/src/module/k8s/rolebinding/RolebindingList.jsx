...
function RolebindingList() {
  ...
  const storeRolebindings = useSelector(store.getRolebindings);
  const storeRolebindingTableLoading = useSelector(store.getRolebindingTableLoading);
  ...
  async function init(context) {
    ...
    try {
      dispatch(store.setRolebindingTableLoading(true));
      ...
      const response2 = await api.get_rolebindings(cluster_id, namespace);
      dispatch(store.setRolebindings(response2.data.data));
      ...
      dispatch(store.setRolebindingTableLoading(false));
    }
  }
  ...
  async function changeCluster(value) {
    ...
    dispatch(store.setRolebindings([]));

    try {
      dispatch(store.setRolebindingTableLoading(true));

      const response = await api.get_rolebindings(cluster_id, namespace);
      dispatch(store.setRolebindings(response.data.data));
  ...
  function getRolebinding(rolebinding) {
    dispatch(store.setRolebinding(rolebinding));
    dispatch(store.setRolebindingDetailVisible(true));
  }

  function getRolebindingYaml(rolebinding) {
    dispatch(store.setRolebinding(rolebinding));
    dispatch(store.setRolebindingYamlVisible(true));
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      sortDirections: ['ascend', 'descend'],
      render: (text, record) => (
        <Button type="link" className="ButtonLink" onClick={() => getRolebinding(record)}>{text}</Button>
      ),
    },
    {
      key: 'role',
    ...
    },
    {
      ...
      render: (text, record) => (
        <div style={{display: 'flex', alignItems: 'center'}}>
          <Button type="link" className="ButtonLink" onClick={() => getRolebindingYaml(record)}>YAML</Button>
        </div>
      ),
    },
  ];
    ...
    <>
      ...
      <div className="MyContentBlock">
        ...
        <Table
          ...
          columns={columns}
          dataSource={storeRolebindings}
          loading={storeRolebindingTableLoading}
