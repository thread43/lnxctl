...
function RolebindingYaml() {
  ...
  async function init() {
    try {
      const rolebinding = storeRolebinding;

      setStateLoading(true);

      const response = await api.get_rolebinding_yaml(rolebinding);
  ...
  return (
    <>
      <Modal
        ...
        width={1000}
        open={storeRolebindingYamlVisible}
        loading={stateLoading}
        onCancel={() => dispatch(store.setRolebindingYamlVisible(false))}
        footer={[
          <Button key="close" onClick={() => dispatch(store.setRolebindingYamlVisible(false))}>Close</Button>,
        ]}
      >
...
export default RolebindingYaml;
