...
function SecretList() {
  ...
  const storeSecrets = useSelector(store.getSecrets);
  const storeSecretTableLoading = useSelector(store.getSecretTableLoading);
  ...
  async function init(context) {
    ...
    try {
      dispatch(store.setSecretTableLoading(true));
      ...
      const response2 = await api.get_secrets(cluster_id, namespace);
      dispatch(store.setSecrets(response2.data.data));
      ...
      dispatch(store.setSecretTableLoading(false));
    }
  }
  ...
  async function changeCluster(value) {
    ...
    dispatch(store.setSecrets([]));

    try {
      dispatch(store.setSecretTableLoading(true));

      const response = await api.get_secrets(cluster_id, namespace);
      dispatch(store.setSecrets(response.data.data));
  ...
  function getSecret(secret) {
    dispatch(store.setSecret(secret));
    dispatch(store.setSecretDetailVisible(true));
  }

  function getSecretYaml(secret) {
    dispatch(store.setSecret(secret));
    dispatch(store.setSecretYamlVisible(true));
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      sortDirections: ['ascend', 'descend'],
      render: (text, record) => (
        <Button type="link" className="ButtonLink" onClick={() => getSecret(record)}>{text}</Button>
      ),
    },
    {
      key: 'type',
    ...
    },
    {
      ...
      render: (text, record) => (
        <div style={{display: 'flex', alignItems: 'center'}}>
          <Button type="link" className="ButtonLink" onClick={() => getSecretYaml(record)}>YAML</Button>
        </div>
      ),
    },
  ];
    ...
    <>
      ...
      <div className="MyContentBlock">
        <div className="MyContentHeader">
          <span className="MyContentHeaderTitle">Secret List</span>
          <Space>
        ...
        <Table
          ...
          columns={columns}
          dataSource={storeSecrets}
          loading={storeSecretTableLoading}
