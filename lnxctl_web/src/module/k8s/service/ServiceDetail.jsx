    ...
    <>
      <Modal
        ...
        footer={[
          <Button
            key="close"
            onClick={() => dispatch(store.setServiceDetailVisible(false))}
          >
            Close
          </Button>,
        ]}
      >
        ...
        >
          ...
          <Form.Item label="Cluster IP">{storeService.cluster_ip}</Form.Item>
          <Form.Item label="Ports">
            {storeService.ports.map((item, index) => {
              if (item.node_port !== 0) {
                return (
                  <div key={index}>
          ...
          <Form.Item label="Age">{storeService.age}</Form.Item>
        </Form>
      </Modal>
    </>
  );
