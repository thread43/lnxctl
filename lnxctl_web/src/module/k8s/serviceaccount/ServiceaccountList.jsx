...
function ServiceaccountList() {
  ...
  const storeServiceaccounts = useSelector(store.getServiceaccounts);
  const storeServiceaccountTableLoading = useSelector(store.getServiceaccountTableLoading);
  ...
  async function init(context) {
    ...
    try {
      dispatch(store.setServiceaccountTableLoading(true));
      ...
      const response2 = await api.get_serviceaccounts(cluster_id, namespace);
      dispatch(store.setServiceaccounts(response2.data.data));
      ...
      dispatch(store.setServiceaccountTableLoading(false));
    }
  }
  ...
  async function changeCluster(value) {
    ...
    dispatch(store.setServiceaccounts([]));

    try {
      dispatch(store.setServiceaccountTableLoading(true));

      const response = await api.get_serviceaccounts(cluster_id, namespace);
      dispatch(store.setServiceaccounts(response.data.data));
  ...
  function getServiceaccount(serviceaccount) {
    dispatch(store.setServiceaccount(serviceaccount));
    dispatch(store.setServiceaccountDetailVisible(true));
  }

  function getServiceaccountYaml(serviceaccount) {
    dispatch(store.setServiceaccount(serviceaccount));
    dispatch(store.setServiceaccountYamlVisible(true));
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      sortDirections: ['ascend', 'descend'],
      render: (text, record) => (
        <Button type="link" className="ButtonLink" onClick={() => getServiceaccount(record)}>{text}</Button>
      ),
    },
    {
      key: 'secrets',
    ...
    },
    {
      ...
      render: (text, record) => (
        <div style={{display: 'flex', alignItems: 'center'}}>
          <Button type="link" className="ButtonLink" onClick={() => getServiceaccountYaml(record)}>YAML</Button>
        </div>
      ),
    },
  ];
    ...
    <>
      ...
      <div className="MyContentBlock">
        <div className="MyContentHeader">
          <span className="MyContentHeaderTitle">ServiceAccount List</span>
          <Space>
        ...
        <Table
          ...
          columns={columns}
          dataSource={storeServiceaccounts}
          loading={storeServiceaccountTableLoading}
